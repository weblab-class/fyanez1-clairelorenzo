...
export function do_colors_match(color, colors_to_match) {
  ...
  for (
    var color1, _pj_c = 0, _pj_a = colors_to_match, _pj_b = _pj_a.length;
    _pj_c < _pj_b;
    _pj_c += 1
  ) {
    color1 = _pj_a[_pj_c];
...
export function make_outfit(items, constraints, items_filled) {
  ...
  for (
    var element, _pj_c = 0, _pj_a = ["top", "bottom", "shoes", "dress"], _pj_b = _pj_a.length;
    _pj_c < _pj_b;
    _pj_c += 1
  ) {
    element = _pj_a[_pj_c];
  ...
  if (options["dress"] === []) {
    ...
    top_ind = getRandomInt(0, number_of_tops);
    // bottom_ind = random.randint(0, number_of_bottoms - 1);
    bottom_ind = getRandomInt(0, number_of_bottoms);
    // shoes_ind = random.randint(0, number_of_shoes - 1);
    shoes_ind = getRandomInt(0, number_of_shoes);
    ...
    if (options["top"] || options["bottom"] === []) {
      ...
      //   shoes_ind = random.randint(0, number_of_shoes - 1);
      dress_ind = getRandomInt(0, number_of_dresses);
      shoes_ind = getRandomInt(0, number_of_shoes);
